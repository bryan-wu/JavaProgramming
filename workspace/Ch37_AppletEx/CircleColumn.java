...
public class CircleColumn extends JApplet
{
    ...
    public class Display extends JPanel
    {
        public void paintComponent( Graphics gr )
        {
            ...
            int diameter = 20;
            int X = width/2 - diameter/2;      // the top edge of the squares
            int shift = (height/10-diameter)/2;  // shift circles right

            int count =  0 ;
            while (  count < 10  )
            {
              int Y = count*height/10+shift;          // the left edge of each square
