...
public class Ch47ArrayAlgorithmsEx1_6
{
    ...
    public int[] Ex2_TwoLargest( int[] data )
    {
        ...
        for ( int index = 0; index < data.length; index++ )
        {
            if ( data[index] > largest ) // find largest
            {
                largest = data[index];
            }

            if ( data[index] > nextLargest && data[index] < largest )
            {
    ...
    {
        int[] result = new int[data.length]; // make reversed array same length
                                             // as data array
        ...
        {
            temp = data[j];
            result[j] = data[dataLength];
            result[dataLength] = temp;
            dataLength--;
        }
    ...
    {
        int[] smooth = new int[signal.length]; // make smooth array same length
                                               // as signal

        // compute the smoothed value for each cell of the array smooth
        smooth[0] = ( signal[0] + signal[1] ) / 2;
        smooth[signal.length - 1] = ( signal[signal.length - 1] 
                        + signal[signal.length - 2] ) / 2; // avg last value
        ...
        {
            System.out.print( "signal: " + signal[j] + " " );
        }

        // write out the result
        for ( int j = 0; j < smooth.length; j++ )
        {
            System.out.print( "smooth: " + smooth[j] + " " );
        }
    ...
    {
        File input = new File( "ph.txt" ); // get input file
        scan = new Scanner( input );

        int index = 0;
        int numberOfValues = 0;
        numberOfValues = scan.nextInt(); // get number of values

        double sum = 0;
        double[] coffee = new double[numberOfValues]; // array for coffee pH's
        ...
        {
            while ( index < coffee.length )
            {
                coffee[index] = scan.nextDouble();
                sum = sum + coffee[index]; // prepare a sum for averaging
                index++;
            }
        }
        mostDistant = coffee[0];
        average = sum / numberOfValues; // here's the average

        // now to find the value farthest from the average

        for ( double val : coffee )
        {
            if ( Math.abs( val - average ) > Math.abs( mostDistant - average ) )
                mostDistant = val;
        }
        sum = sum - mostDistant; // subtract the value farthest from the avg
        newAverage = sum / ( numberOfValues - 1 ); // new average!!
