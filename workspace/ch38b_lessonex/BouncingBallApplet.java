 ...
 */
public class BouncingBallApplet extends SimpleAnimationApplet2
{
    BouncingBall redBall;

    /**
     * This routine is called by the system when the applet is
    ...
    public void init()
    {
        int w = getWidth();   // Applet width, to be used for positioning the ball.
        int h = getHeight();  // Applet height, to be used for positioning the ball.

        redBall = new BouncingBall(w / 2, h / 2, 15, Color.red);

        // This routine is defined in SimpleAnimationApplet. It says
        // that I want each frame to take 50 milliseconds (1/20 second).
        // This is about twice as fast as the default rate.  (Note that
        // there is no guarantee that the computer can actally draw frames
        // at the rate you request.)
