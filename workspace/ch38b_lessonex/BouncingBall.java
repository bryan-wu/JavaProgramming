...
public class BouncingBall extends Circle 
{
    ...
    /**
      ...
      components of the velocity re random numbers between -10 and 10,
      except that at least one of the components must be greater than 3
      in absolute value.
   ...
   // int width = getWidth();
    ...
    public BouncingBall( int centerX, int centerY, int radius, Color color )
    {
        ...
        while ( count < 10 )
        {
            ...
            radius = Math.abs( rand.nextInt() ) % 20 + 5;
            //centerX = Math.abs( rand.nextInt( width - 2 * radius ) ) % 400;
           // centerY = Math.abs( rand.nextInt( height - 2 * radius ) ) % 400;
    ...
    /**
      ...
      the velocity to make the ball head back into the rectangle. This method is
      meant to be called once for each frame of the animation.
    ...
    /**
      Modify the velocity so that the ball is moving in the direction of the
      point (x, y). The speed of the ball does not change, only its direction
      of motion. (If the specified point is too close to the current position
      of the ball, the velocity of the ball does not change.) This method
      uses some standard vector mathematics.
     */
    public void headTowards( int x, int y )
    {
        if ( Math.abs( x - getX() ) < 1.0E-10 && Math.abs( y - getY() ) < 1.0E-10 )
        {
            return;
        }
